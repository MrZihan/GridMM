...
class RxRInstructionSensor(Sensor):
    ...
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained('data/pretrained_models/xlm')
        super().__init__(config=config)
    ...
    def get_observation(
        ...
        **kwargs,
        ...
        instruction_ids = np.zeros((500,), dtype=np.int32)
        instruction = self.tokenizer(episode.instruction.instruction_text)['input_ids']
        instruction = np.array(instruction)
        instruction_ids[:instruction.shape[-1]] = instruction[:500]
