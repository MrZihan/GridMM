...
@baseline_registry.register_trainer(name="schedulesampler-GridMap")
class SSTrainer(BaseVLNCETrainer):
    ...
    def train_ml(self, in_train=True, train_tf=False, train_rl=False):
        ...
        for stepk in range(self.max_len):
            ...
            if stepk == 0:
                ...
                policy_net.max_x = [-10000 for i in range(batch_size)]
                policy_net.min_x = [10000 for i in range(batch_size)]
                policy_net.max_y = [-10000 for i in range(batch_size)]
                policy_net.min_y = [10000 for i in range(batch_size)]
                policy_net.global_map_index = [[] for i in range(batch_size)]
                policy_net.traj_embeds = [[] for i in range(batch_size)]
                policy_net.traj_map = [[] for i in range(batch_size)]
            ...
            if 'GridMap' in self.config.MODEL.policy_name:
                ...
                logits = self.policy.net(
                    ...
                    in_train = in_train,
                    lang_feats=language_features, candidate_lengths=candidate_lengths, batch_angles=batch_angles, batch_distances=batch_angles,
                    batch_view_img_fts=batch_view_img_fts,positions = positions,
                    batch_loc_fts=batch_loc_fts, batch_nav_types=batch_nav_types, batch_view_lens=batch_view_lens, batch_grid_fts=batch_grid_fts, batch_map_index=batch_map_index,batch_gridmap_pos_fts=batch_gridmap_pos_fts
                )
    ...
    def train(self) -> None:
        ...
        self.config.NUM_ENVIRONMENTS = 1#self.config.IL.batch_size // len(
