...
class GlocalTextPathCMTPreTraining(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if 'sap' in config.pretrain_tasks:
            ...
            self.grid_sap_head = ClsPrediction(self.config.hidden_size)
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        if task.startswith('mlm'):
            return self.forward_mlm(
                ...
                batch['txt_labels'], batch['grid_fts'],batch['grid_map'], batch['target_patch_id'],batch['gridmap_pos_fts'], compute_loss
            )
            ...
            return self.forward_mrc(
                ...
                batch['vp_obj_mrc_masks'], batch['vp_obj_probs'], batch['grid_fts'],batch['grid_map'], batch['target_patch_id'],batch['gridmap_pos_fts'], compute_loss
            )
            ...
            return self.forward_sap(
                ...
                batch['global_act_labels'], batch['local_act_labels'],batch['grid_fts'],batch['grid_map'], batch['target_patch_id'], batch['gridmap_pos_fts'], compute_loss
            )
            ...
            return self.forward_og(
                ...
                batch['gmap_lens'], batch['gmap_step_ids'], batch['gmap_pos_fts'], 
                batch['gmap_pair_dists'], batch['gmap_vpids'], batch['vp_pos_fts'],
                batch['obj_labels'], batch['grid_fts'],batch['grid_map'], batch['target_patch_id'],batch['gridmap_pos_fts'],compute_loss
            )
            ...
            return self.forward_sap_og(
                ...
                batch['obj_labels'],batch['grid_fts'],batch['grid_map'], batch['target_patch_id'],batch['gridmap_pos_fts'],
            )
        else:
    ...
    def forward_mlm(
        ...
        txt_embeds = self.bert.forward_mlm(
            ...
            traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
            gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,grid_fts, grid_map,gridmap_pos_fts
        )
    ...
    def forward_mrc(
        ...
        traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
        gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,
        vp_view_mrc_masks, vp_view_probs, vp_obj_mrc_masks, vp_obj_probs, grid_fts, grid_map,target_patch_id, gridmap_pos_fts,compute_loss=True
    ):

        _, vp_embeds, _ = self.bert(
            txt_ids, txt_lens, traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, 
            traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
            gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,grid_fts, grid_map,gridmap_pos_fts=gridmap_pos_fts,
    ...
    def forward_sap(
        ...
        traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
        gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,
        gmap_visited_masks, global_act_labels, local_act_labels, grid_fts, grid_map,target_patch_id, gridmap_pos_fts, compute_loss
    ):
        ...
        if compute_loss:
            ...
            if global_losses[global_act_labels!=0].shape[0] != 0:
                stop_rate = global_losses[global_act_labels==0].shape[0] / global_losses[global_act_labels!=0].shape[0]
            else:
                stop_rate = 1.
    ...
    def forward_sap_og(
        ...
        traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
        gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,
        gmap_visited_masks, global_act_labels, local_act_labels, obj_labels, grid_fts, grid_map,target_patch_id, gridmap_pos_fts
    ):
        batch_size = txt_ids.size(0)
