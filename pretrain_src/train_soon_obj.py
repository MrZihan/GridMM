...
from data.dataset import SoonTextPathData
...
def main(opts):
    ...
    train_nav_db = SoonTextPathData(
        ...
        max_txt_len=opts.max_txt_len, max_objects=opts.max_objects, in_memory=False, is_train=True
    )
    val_nav_db = SoonTextPathData(
        ...
        obj_feat_size=model_config.obj_feat_size,
        obj_prob_size=model_config.obj_prob_size,
        max_txt_len=opts.max_txt_len, max_objects=opts.max_objects, in_memory=False, is_train=False
    )
...
def validate_mlm(model, val_loader):
    ...
    for i, batch in enumerate(val_loader):
        ...
        loss = F.cross_entropy(scores, labels.to(torch.int64), reduction='sum')
        val_loss += loss.item()
...
def validate_sap(model, val_loader):
    ...
    for i, batch in enumerate(val_loader):
        ...
        val_gloss += F.cross_entropy(global_logits, global_act_labels.to(torch.int64), reduction='sum').data.item()
        val_lloss += F.cross_entropy(local_logits, local_act_labels.to(torch.int64), reduction='sum').data.item()
        val_floss += F.cross_entropy(fused_logits, global_act_labels.to(torch.int64), reduction='sum').data.item()
